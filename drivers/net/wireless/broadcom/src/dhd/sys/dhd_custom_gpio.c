...
dhd_customer_gpio_wlan_reset(bool onoff)
{
	if (onoff == G_WLAN_SET_OFF) {
...
#ifdef ANDROID_SPECIFIC
		 /* Pins are active low. 0 implies RESET state */
		...
		WL_ERROR(("%s: call customer specific GPIO to remove WLAN RESET\n", __FUNCTION__));
#ifdef ANDROID_SPECIFIC
		 gpio_direction_output(BCM4325_WLAN_RESET, 1);
		 msleep(200);
#endif
