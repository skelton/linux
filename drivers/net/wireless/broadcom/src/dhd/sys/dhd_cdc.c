...
static dhd_pub_t *powerdhd = NULL;
int dhd_deepsleep(int flag) 
{
	char iovbuf[20];
	uint powervar = 0;
	...
	switch (flag) {
		case 1 :  /* Deepsleep on */
			printk(KERN_INFO "[WiFi] Deepsleep On \n");
			/* Disable MPC */
			powervar = 0;
			bcm_mkiovar("mpc", (char *)&powervar, 4, iovbuf, sizeof(iovbuf));
			dhdcdc_set_ioctl(powerdhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		...
		case 0: /* Deepsleep Off */
			...
			printk(KERN_INFO "[WiFi] Deepsleep Off\n");
			/* Disable Deepsleep */
			powervar = 0;
			bcm_mkiovar("deepsleep", (char *)&powervar, 4, iovbuf, sizeof(iovbuf));
			dhdcdc_set_ioctl(powerdhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
