 ...
 *
 * $Id: dhd_linux.c,v 1.65.4.9.2.13.6.30 2009/04/17 05:44:28 Exp $
 */
...
static int	dhd_set_txglom(dhd_pub_t *pub, int ifidx);
#endif
...
static int dhd_write_macaddress(char *macaddr)
{
	...
	/*
	 * File Doesn't Exist. Create File and write Mac address. This activity is only required for the first time.
	 */
...
#if 0 /* Else part for Debugging */
	else
	{
		/*
		 * File exists. so do nothing..
		 * */
		printk(KERN_INFO "[WIFI] %s file Exists!! Do Nothing..\n", filepath);
	}
#endif
	 set_fs(oldfs);
...
#ifdef ANDROID_SPECIFIC
	/*write Mac Addr to external file for the first time*/
	dhd_write_macaddress(dhd->pub.mac.octet);
#endif
...
dhd_detach(dhd_pub_t *dhdp)
{
	...
	if (dhd) {
		...
		dhd_net_stop(dhdp);	/* Copy this logic from RC31 :SW.LEE */
