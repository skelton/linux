 ...
 *
 * $Id: dhd_sdio.c,v 1.157.2.27.2.36.4.42 2009/04/17 05:55:29 Exp $
 */
...
dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "hostintmask 0x%08x intstatus 0x%08xn sdpcm_ver %d\n",
...
/* To check whether firmware download is good or not */
static uint cnt_chipactive = 0;
#endif
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	if (intstatus & I_CHIPACTIVE) {
		...
		if (!cnt_chipactive) {
			cnt_chipactive=1;
			printk(KERN_INFO "[WiFi] Dongle reports CHIPACTIVE\n");
		}
#endif
	}
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	if (dhd_net_start(bus->dhd) != 0) {
		DHD_ERROR(("%s: dhd_net_start failed!\n", __FUNCTION__));
		goto fail;
	}
