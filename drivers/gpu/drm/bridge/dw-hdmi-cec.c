...
static inline void hdmi_write(struct dw_hdmi_cec *cec,
			      u8 val, int offset)
{
	struct dw_hdmi *hdmi = cec->hdmi;

	cec->ops->write(hdmi, val, offset);
}
...
static inline u8 hdmi_read(struct dw_hdmi_cec *cec, int offset)
{
	...
	return cec->ops->read(hdmi, offset);
}
...
static int dw_hdmi_cec_probe(struct platform_device *pdev)
{
	...
	cec->n = hdmi_notifier_get(cec->adap->devnode.dev.parent);
	if (!cec->n)
		return -ENOMEM;
	cec_register_hdmi_notifier(cec->adap, cec->n);
