...
static void htc_fb_console_update(void)
{
	...
	writel((height << 16) | (width), MSM_MDP_BASE + MDP_FULL_BYPASS_WORD33);
	writel(fbram, MSM_MDP_BASE + MDP_FULL_BYPASS_WORD34);
	writel(stride, MSM_MDP_BASE + MDP_FULL_BYPASS_WORD35);

	/* 666 18BPP */
	dma2_cfg |= DMA_DSTC0G_6BITS | DMA_DSTC1B_6BITS | DMA_DSTC2R_6BITS;

	/* set y & x offset and MDDI transaction parameters */
	writel((y << 16) | (x), MSM_MDP_BASE + MDP_FULL_BYPASS_WORD37);
	writel(ld_param, MSM_MDP_BASE + MDP_FULL_BYPASS_WORD40);
	writel((MDDI_VDO_PACKET_DESC << 16) | MDDI_VDO_PACKET_PRIM, MSM_MDP_BASE + MDP_FULL_BYPASS_WORD41);

	writel(dma2_cfg, MSM_MDP_BASE + MDP_DMA_CONFIG);
...
static int __init htc_fb_console_init(void)
{
	struct map_desc map;
